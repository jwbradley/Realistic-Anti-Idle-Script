...
| 2 | F15 key press | Only when idle (120s+ no input), every 60-90s | Zero — no application responds to F15 |
| 3 | Mouse jiggle + keyboard | Only when idle (120s+ no input) | None while you're working |

**You can leave this running 24/7 while actively working.** Only Layer 1 (the invisible OS flag) runs while you're active. Both Layer 2 (F15) and Layer 3 (mouse/keyboard) are gated behind the same idle check and only engage after you've been away for 2+ minutes.
...
- **Layer 2 (F15):** Resets the Windows idle timer at the HID level. F15 exists in the keyboard spec but no application uses it — completely invisible. Only fires once you've been idle (no keyboard or mouse) for the threshold, so it never interferes with a window you're actively working in.
...
USER_IDLE_THRESHOLD = 120       # Seconds of no keyboard/mouse before F15 + jiggle activate
...
Every 10 seconds:
  ...
  └── If no mouse AND no keyboard for 120s → user is idle
      └── Layers 2 + 3 fire (F15 + mouse jiggle)
```
...
- **Only Layer 1 (SetThreadExecutionState) runs while you're active** — it's a pure OS flag with zero observable behavior. F15 and the mouse jiggle stay dormant until you've been idle 120s.
...
============================================================
  ...
  - Layer 2: F15 key tap (invisible, only when idle)
...
[09:17:30] User active — layer 1 only (cycle #45)
[09:22:10] Idle 130s — mouse jiggle + F15 active (cycle #75)
[09:25:40] User active — layer 1 only (cycle #96)
```
...
> Note: Layers 2 and 3 only fire after `USER_IDLE_THRESHOLD` (120s) of no input. While you're actively working, only Layer 1 is running — but you don't need the others then, since your own activity already keeps Teams/Slack "Active" and the idle timers reset.
...
| Invisible layers | No | Yes (OS flag always on; F15 invisible, only when idle) |
